...
class simple_html_dom_node {
    ...
    function text() {
        ...
        $ret = '';
        // In rare cases, (always node type 1 or HDOM_TYPE_ELEMENT - observed for some span tags, and some p tags)
        // $this->nodes is set to NULL.
    ...
    protected function seek($selector, &$ret, $lowercase = false) {
        ...
        for($i = $this->_[HDOM_INFO_BEGIN] + 1; $i < $end; ++$i) {
            ...
            if($pass && $key && $val && $val !== '*') {
                ...
                if(is_object($debug_object)) {
                    $debug_object->debug_log(2, "testing node: " . $node->tag . " for attribute: "
                ...
                if(!$check && strcasecmp($key, 'class') === 0) {
                    foreach(explode(' ', $node->attr[$key]) as $k) {
                        // Without this, there were cases where leading, trailing,
                        // or double spaces lead to our comparing blanks - bad form.
                        if(!empty($k)) {
                            if($lowercase) {
...
class simple_html_dom {
    ...
    protected $self_closing_tags = array(
        'img'    => 1, 'br' => 1, 'input' => 1, 'meta' => 1, 'link' => 1, 'hr' => 1, 'base' => 1, 'embed' => 1,
        'spacer' => 1
    );
    ...
    protected function parse_charset() {
        ...
        if(empty($charset)) {
            ...
            // and if this doesn't work...  then we need to just wrongheadedly assume
            // it's UTF-8 so that we can move on - cause this will usually give us most of what we need...
            if($charset === false) {
                if(is_object($debug_object)) {
    ...
    protected function parse_attr($node, $name, &$space) {
        ...
        // PaperG: If this is a "class" selector, lets get rid of the preceeding and trailing space
        // since some people leave it in the multi class case.
        if($name == "class") {
