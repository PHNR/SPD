...
defined('PCLZIP_READ_BLOCK_SIZE') || define('PCLZIP_READ_BLOCK_SIZE', 2048);
defined('PCLZIP_SEPARATOR') || define('PCLZIP_SEPARATOR', ',');
defined('PCLZIP_ERROR_EXTERNAL') || define('PCLZIP_ERROR_EXTERNAL', 0);
defined('PCLZIP_TEMPORARY_DIR') || define('PCLZIP_TEMPORARY_DIR', '');
defined('PCLZIP_TEMPORARY_FILE_RATIO') || define('PCLZIP_TEMPORARY_FILE_RATIO', 0.47);
...
class PclZip {
	...
	function errorCode() {
		if(PCLZIP_ERROR_EXTERNAL == 1) return (PclErrorCode()); else return ($this->error_code);
	}
	...
	function errorName($p_with_code = FALSE) {
		...
		if($p_with_code) return ($v_value.' ('.$this->error_code.')'); else return ($v_value);
	}



	function errorInfo($p_full = FALSE) {
		if(PCLZIP_ERROR_EXTERNAL == 1) return (PclErrorString()); else {
			if($p_full) return ($this->errorName(TRUE)." : ".$this->error_string); else return ($this->error_string." [code ".$this->error_code."]");
		}
	}
	...
	function privDeleteByRule(&$p_result_list, &$p_options) {
		$v_result      = 1;
		$v_list_detail = array();
		if(($v_result = $this->privOpenFd('rb')) != 1) return $v_result;
		$v_central_dir = array();
		if(($v_result = $this->privReadEndCentralDir($v_central_dir)) != 1) {
			$this->privCloseFd();
		...
		if($v_nb_extracted > 0) {
			...
			$this->privCloseFd();
			if(($v_result = $this->privOpenFd('wb')) != 1) return $v_result;
			if(($v_result = $this->privWriteCentralHeader(0, 0, 0, '')) != 1) return $v_result;
			$this->privCloseFd();
		}
