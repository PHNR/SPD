 ...
 *    isTextAdded()     - returns TRUE/FALSE               (`plain/text', `Charset' values added to $lists)
 *
 ...
 *
 * @category File
 * @version $Id: 7z.include, v0.13.1 2010-03-28
        ...
        if(
            ...
            foreach($this->lists['Path'] as $i => $path) {
                ...
                if(
                    ...
                    // no `Folder' in $lists, and we have the means to add it
                    if(
                        (
                            (!$noAttributes) and
                            (strtolower($this->lists['Attributes'][$i][0]) == 'd')
                        ) or (
                            ($this->lists['Size'][$i] < 1) and
                            ($this->lists['Packed Size'][$i] < 1)
                        )
                    ) {
                        // this is a directory
                        ...
                        if((!$add_mime) and ($this->lists['Size'][$i] < 1)) {
                            // do the best we can
                            if(isset($this->lists['Packed Size'][$i])) {
                                $this->lists['Size'][$i] = $this->lists['Packed Size'][$i];
                            }
                        }
                    }
                } // if( $noFolder and...)

                if($noAttributes) $this->lists['Attributes'][$i] = '';
                if($noComment) $this->lists['Comment'][$i] = '';
                if($noCRC) $this->lists['CRC'][$i] = '';
                if($noEncrypted) $this->lists['Encrypted'][$i] = false;
                if($noHost_OS) $this->lists['Host OS'][$i] = '';
                if($noMethod) $this->lists['Method'][$i] = '';
                if($noModified) $this->lists['Modified'][$i] = '';
                if($noPackedSize) $this->lists['Packed Size'][$i] = 0;
                if($noSize) $this->lists['Size'][$i] = 0;
                            ...
                            if($add_text) {
                                if($type == 'text/rtf') {
                                    ...
                                    if($err or empty($lines)) {
                                        ...
                                        $this->_charset_added       = true;
                                        $this->lists['Charset'][$i] = 'iso-8859-1';
                                    }
        ...
        foreach($lines as $key => $line) {
            ...
            if(strpos($line, ' =') === false) {
                ...
                switch($k) {
                    ...
                    case 'Folder':
                    case 'Encrypted':
                        $this->lists[$k][$i] = (($v == '1') or ($v == '+'));
                        break;
                    case 'Size':
                    case 'Packed Size':
                        $this->lists[$k][$i] = ( int )$v;
                    ...
                    case 'Host OS':
                    case 'Attributes':
                        if(($k == 'Attributes') and (strtolower($v[0]) == 'd')) $hasDir = true;
                    case 'Comment':
                        if($i == -1) { // begin archive-wide comment
                            $this->_comment = $v;
                            continue;
                        }
                    case 'Method':
                        $this->lists[$k][$i] = ( string )$v;
                        break;
